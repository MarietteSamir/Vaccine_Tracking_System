...
#define ALL_RECORDS_TEXT_FILE_PATH "MapRecord.csv"
#define Waitinglist_TEXT_FILE_PATH "Waitinglist.csv"
...
void MaskingPswd(string& Pswd)  //DONE      MARIETTE
{
...
void DisplayRecord(long long Id)   // DONE      ALI
{
    bool found = false;
    for (auto it = Records.begin(); it != Records.end(); it++)
    {
    ...
    while (true)
    {
        ...
        case 5:
            while (true) {      //DosesReceived Validation
                cout << "How Many Doses Have You Received? (0/1/2)\n";
                cin >> TestRecord.DosesReceived;
                if ((Records.find(ID)->second.DosesReceived == 0 && TestRecord.DosesReceived == 1) || ((Records.find(ID)->second.DosesReceived == 1 || Records.find(ID)->second.DosesReceived == 0) && TestRecord.DosesReceived == 2))
                {
...
void AdminMenu()        
{
    ...
    cout << "2. Delete all any record(s) by entering only National ID.\n";
...
void AgeGroupInfo() 
{
    do
    {
        ...
        for (auto it = Records.begin(); it != Records.end(); it++)
        {
            if (it->second.Age >= MinAge && it->second.Age <= MaxAge)
            {
                Counter++;
                if (it->second.Gender == "M" || it->second.Gender == "m")
                {
...
void main()
{
    ...
    while (true)
    {
        ...
        case 2:
            ...
            for (auto it = Records.begin(); it != Records.end(); it++)
            {
                if (it->first == NationalID && it->second.Pswd == Password)
                {
                    while (true)
                    {
                        bool exit = false;
